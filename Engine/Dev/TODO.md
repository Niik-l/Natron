...
- [ ] **Linux follow-ups** (Linux port verified 2026-09-20: 9-distro CI matrix, `linux-portable-latest` pre-release, RTX 4090 pod session):
  - [ ] **Attach the Linux AppImage to each numbered release** (same commit as the Windows package; run Linux Portable on that commit, download the AppImage + debug-symbols from the run or from the pre-release while it still points at that commit, `gh release upload <tag> ...`). Then the rolling pre-release is the nightly and users who want a fixed version have one. Noted in BUILDING.md (Linux section) and the local PACKAGING_NOTES.md.
