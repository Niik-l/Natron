...
- [x] **Generic per-knob lock (right-click → Lock Parameter)** — DONE 2026-07-27 (KnobHelper::setUserLocked/isUserLocked, serialization v15, context-menu entry; ticked 2026-09-07 during a list audit). ORIGINAL NOTE: — Maya/Houdini-style: lock ANY knob against user edits from the knob context menu. Needs a new persistent "user locked" flag on KnobHelper (SEPARATE from `enabled` — nodes drive enabled programmatically and would fight a user-lock), serialization (backward-compatible new field), the KnobGui context-menu entry, and an edit-path audit (panel, curve editor / dope sheet key drags, paste, links). Once in, the node-level Lock Transform (Camera3D / ReadAlembicCamera / ReadAlembicTransform, 2026-07-26) should set the generic lock on its 6-9 knobs so the two unify.
- [x] **Read: an empty Read given a file never guessed its colorspace** — FIXED 2026-09-07 (`ReadNode.cpp`, fresh-decoder branch of `createReadNode`). The mechanism guessed on the morning of 09-07 was wrong (the reason code is fine); traced with file-logging in `KnobHolder::endChanges` / `ReadNode::knobChanged` / `OfxEffectInstance::knobChanged`: the decoder's knobs are HELD BY THE READNODE WRAPPER, so when the decoder is created in response to the file knob changing, its `filename` param binds to the very knob that already holds the new path — `fileKnob->setValue(filename)` is a no-op, no instanceChanged reaches the plugin, and `guessParamsFromFilename` never runs (no colorspace / premult / components / frame-range guess; `ParamExistingInstance` stayed false). `createReader` never hit it because the wrapper is created with the filename and the decoder's knob starts empty. Fix: when the knob already held the path, call `onKnobValueChanged_public` on the decoder explicitly (mirrors the same-decoder branch), skipped on project load so serialized first/last frame are not clobbered. Affected every decoder, not just ReadOIIO (ReadPNG only looked right because its default is sRGB).
- [x] **Read/Write under an ACES config failed: `Color space 'default' could not be found`** — FIXED 2026-09-09 in the openfx-io patch (GenericOCIO fallback helper; it was the plugin's reset-to-default path, not WritePNG specifically — ReadPNG hit it too). ORIGINAL NOTE: (2026-09-08, seen while testing the Card3D Cycles fix with the ACES 2.0 CG config active). The PNG writer's default output colorspace resolves to OCIO's `default` ROLE, which the ACES cg/studio configs do not define, so `Write` to .png errors out until the user picks a colorspace by hand. Same family as the 09-07 OIIO-3 name fixes: `openfx-io` `PNG/WritePNG.cpp` (and check WriteOIIO / other writers) should fall back through the ACES texture/display names when the role is missing. Fix goes in `tools/win-build/patches/openfx-io.patch`.
- [x] **Project saved under one OCIO config failed under another: `Color space 'sRGB' could not be found`** — FIXED 2026-09-09. The user's 09-08 scene (built before the 09-09 plugin swap) stored the PNG Read's colorspace as `sRGB`; the ACES 2.0 studio config has no such name or alias, and openfx-io only validated the stored name on a user edit, so the project loaded with a text field and every render of that Read errored. Fix in the openfx-io patch (`GenericOCIO.cpp`: `kColorSpaceSynonyms` groups + `resolveColorSpaceName()` applied to every read of the input/output name and in `changedParam` for any reason) plus `ReadNode::onKnobsLoaded()` firing the decoder's changedParam after the values are restored so the menu shows the resolved name. Verified headlessly on the user's scene (Read2 → `sRGB - Texture`, renders) and both ways synthetically (`sRGB` under ACES → `sRGB - Texture`; `sRGB - Texture` under blender → `sRGB`). Side effect worth knowing: a Python `set("sRGB")` under ACES used to silently fall back to `ACEScg` (wrong colours, no error); it now resolves to the texture space.
- [ ] **Camera3D keyframes land on the wrong frame** (user report 2026-09-08: keys set for f300 ended up on f50; unreproduced, deferred at the user's request — needs a repro: which knob, set via panel or viewport gizmo, timeline vs project frame range, autokey state).
- [ ] **GeoBuilder slice 2 — bake** (planned, user says it can wait): write a placed grid card out as OBJ → ReadGeo, and a one-button Project3D + FrameHold projection setup on the card. Slices 3–4 = vertex editing / multi-frame alignment. The card is already a usable mesh everywhere ReadGeo is.
- [x] **PNG sequence dropped into a fresh Natron still said `Color space 'default' could not be found`** (2026-09-13, dev build). Root cause in the plugin: `canonicalizeColorSpace()` equated an unknown name (index -1) with a role the config lacks (also -1) and returned that role — under ACES the ReadPNG knob's describe-time default (`"sRGB"`, not an ACES name) became the literal `default`, so any Read whose guess did not apply carried it. Fixed in the openfx-io patch (unknown names never canonicalise; ACES texture names in the describe lookup; stale `default` resolves to the config fallback; knob defaults resolved). Verified headlessly: the knob default is now a valid ACES entry, an explicit `default` renders. New IO.ofx installed in Natron-install (copy: `v14\_build\IO.ofx.fixed-0913-defaultrole`).
- [ ] **Next release must ship the rebuilt `IO.ofx`** — the 09.07 package has the old openfx-io plugin; the "default" colorspace fallback (de3e91fab) and the cross-config synonym table (b87386794) live in the plugin, not Natron. Rebuild from `tools/win-build/patches/openfx-io.patch` (BUILDING.md 3b) or copy `v14\_build\IO.ofx.fixed-0913-defaultrole` (supersedes the 0909 copy).
- [x] **Linux: intermittent abort on SIGTERM at shutdown** — FIXED 2026-09-20 (handler now only writes to a socketpair; a QSocketNotifier slot on the main thread does the quit). Verify on the CI matrix `shutdown` column over the next runs; if an abort still shows, the gdb backtrace in the artifact names the QThread owner. ORIGINAL NOTE: (seen 2026-09-20 in the CI distro matrix, Arch, run 35517258559; the same check passed on Arch three runs before). The GUI runs fine; on SIGTERM `handleShutDownSignal` (Engine/AppManager.cpp) calls `appPTR->quitApplication()` straight from signal context, and teardown then hits `QThread: Destroyed while thread is still running` -> exit 134. Fix direction: the handler should only flag/wake the main thread (self-pipe + QSocketNotifier, the Qt-documented pattern) and quit from there; then find which QThread owner is deleted while running. CI now records this separately as `shutdown` (informational) and captures a gdb backtrace when it happens.
- [ ] **Linux follow-ups** (Linux port verified 2026-09-20: 9-distro CI matrix, Linux attached to numbered releases, RTX 4090 pod session):
  - [ ] **Attach the Linux AppImage to each numbered release** (same commit as the Windows package; run Linux Portable on that commit, download the AppImage + debug-symbols from the run or from the pre-release while it still points at that commit, `gh release upload <tag> ...`). (The rolling pre-release was dropped 2026-09-21: releases are numbered only, both platforms, same commit.) Noted in BUILDING.md (Linux section) and the local PACKAGING_NOTES.md.
  - [ ] **openfx-arena + openfx-gmic (both platforms, optional)** — neither platform ships them (the README used to claim Windows did; corrected 2026-09-20). Upstream Natron bundles both (Text, ReadSVG, ReadPSD, G'MIC filters). Windows: MSYS2 has ImageMagick/poppler/librsvg packages, so arena is a normal 05-plugins addition; G'MIC is a very long compile. Linux: build the same deps into /usr/local in the portable image (cached under DEPS_KEY) and add both to `tools/linux/build-plugins.sh`. Do both platforms together so the packages stay identical.
  - [ ] **GPU Cycles (CUDA/OptiX)** — every build passes `-DWITH_CYCLES_DEVICE_CUDA/OPTIX=OFF`. Linux: CUDA toolkit + OptiX SDK in the portable build image, kernels compiled at build time, test on the GPU pod. Windows: harder, nvcc needs MSVC (the MSYS2/MinGW toolchain cannot drive it) — would need a separate MSVC kernel build step. Not required for parity between platforms; a feature decision.
- [ ] **GUI-test the new Preferences > 3D Viewport page** (built 2026-09-21, headless-verified compile only): each nav preset on the free camera AND while looking through an editable Camera3D; box select / point pick / gizmo drag under every preset; right-click Blast menu appears on a plain click and never after a nav drag; background gradient + grid colour apply live to every open 3D viewport and survive a restart; two 3D viewports at once (ImGuizmo file-static caveat); unclamped orbit over the top; Blender preset on the Linux AppImage under a WM that grabs Alt+drag. Not done from the plan: zoom-towards-cursor, gizmo size, point size, per-axis grid colours.
- [ ] Test all deep nodes
...
- [ ] Denoise
- [x] **Uniform Scale on the volume (FastVolumeRender / ReadVDB)** — DONE 2026-09-07: knob added to ReadVDB (min 0.0001, display 0.001–10 like ReadGeo) and folded into `ReadVDB::getTransform`, which is the single path SceneGraph (viewport / Cycles / FastVolumeRender via `sn.worldMatrix`) and ScanlineRender's bbox already use; FastVolumeRender keys its GPU upload on the resulting matrix so no separate key change was needed. ORIGINAL NOTE (2026-08-20, user request): ReadVDB's Transform page has translate/rotate/scaleX-Y-Z (`ReadVDB.cpp:242-296`) but NO **Uniform Scale**, while Card3D / Cube3D / Sphere3D / Cylinder3D / ReadAlembicArchive all have one — so scaling a VDB means editing three knobs in lockstep. **Consumer side is DONE** (2026-08-22, with ReadGeo's Uniform Scale): `SceneGraph::rebuild`'s generic branch and `ScanlineRender::extractGeometry` both multiply `uniformScale` into the per-axis scales now, read by name — so any node that grows the knob is honoured by the viewport, ScanlineRender and Cycles. REMAINING for ReadVDB: just add the knob itself, then check FastVolumeRender separately since it builds its own volume matrix (`FastVolumeRender.cpp` volMatrix / upload key — the key must include the new knob or a scale change won't re-upload the GPU bricks).
- [x] **ReadVDB frame-offset UX + VDB error reporting** — ALL THREE DONE 2026-08-21 (a6ed2a97c: detectSequence, wider offset range + tooltip, exception into the persistent message; shipped in 08.24; ticked 2026-09-07). ORIGINAL NOTE: (2026-08-15, diagnosed from a release-build report — engine verified correct, both items are discoverability):
  - [x] **Single-frame VDBs are broken by frame substitution** (CONFIRMED 2026-08-20 from a user repro: `ReadVDB1 → FastVolumeRender1`, playhead at frame 1, error `failed to load VDB grids` while the 3D viewport happily previews the same volume). `resolveFramePath` rewrites the last digit group before `.vdb` UNCONDITIONALLY, so a one-off `smoke_0000.vdb` becomes `smoke_0001.vdb`, and even a version suffix like `fire_v003.vdb` becomes `fire_v001.vdb`. Only a digitless name (`smoke.vdb`) survives. The viewport/render split is because the preview paths fall back to the template when the resolved file is missing (`:576-583`, `:650-656`) and the render paths don't (`:766`, `:853`). **Fix at the root, not with another fallback** (falling back would silently render the wrong frame of a real sequence): decide sequence-ness when the file is chosen — does the pattern match >1 file on disk? — and only substitute frames when it genuinely is one. That fixes this AND the timeline-1001-over-a-0000-sequence case together.
  - [x] `frameOffset` knob (`ReadVDB.cpp:221-226`): display range is ±100 so the slider can't reach a real shot offset (timeline at 1001 over a `_0000`-based sequence needs −1000; typing works, dragging doesn't). Widen it, and add the tooltip this knob alone is missing — `file frame = current frame + offset`, with the 1001 → −1000 example. Note `resolveFramePath()` rewrites the last digit group before `.vdb` unconditionally, so ANY numbered filename is treated as a sequence.
  - [x] Error text: `getVDBDirect()` / `getVolumeData()` swallow the OpenVDB exception to stderr (`ReadVDB.cpp:835`), so FastVolumeRender (and the Cycles volume path, same getter) can only report a bare "failed to load VDB grids." Put the resolved path + exception into the persistent message — a GUI launch never sees stderr, which is what made this undiagnosable. Deliberately NOT adding the missing-frame fallback `getBounds()` has (`ReadVDB.cpp:576-583`): silently rendering the wrong frame is worse than an error.
