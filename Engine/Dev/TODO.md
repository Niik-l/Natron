...
- [ ] **Read: an empty Read given a file never guesses its colorspace** (2026-09-07, found while fixing the OIIO-3 colorspace-name bug in `tools/win-build/patches/openfx-io.patch`; reproduced headlessly on the 08.05 and 08.24 releases, so it is pre-existing). `app.createNode("fr.inria.built-in.Read")` then `filename.set("x.jpg")` leaves File Colorspace at `scene_linear`; `app.createReader("x.jpg")` gets `sRGB`. Mechanism: the empty Read already holds the default decoder (ReadOIIO), so `ReadNodePrivate::createReadNode` takes its "same plug-in, do not create a new decoder" branch and forwards the file change with `eValueChangedReasonNatronInternalEdited`; openfx-io's `GenericReaderPlugin::changedParam` only calls `guessParamsFromFilename` when the reason is `eChangeUserEdit`, so the guess never runs. PNG is unaffected only because its decoder (ReadPNG) differs from the default and gets created fresh. GUI-reachable via cancel-the-dialog-then-browse and via Python. Fix candidates: forward a user reason from that branch (check `ReadNode::knobChanged` doesn't re-enter), or have GenericReader guess whenever `_guessedParams` is still false and the reason is not `eChangeTime`.
- [ ] Test all deep nodes
