...
- [ ] **Linux follow-ups** (Linux port verified 2026-09-20: 9-distro CI matrix, `linux-portable-latest` pre-release, RTX 4090 pod session):
  ...
  - [ ] **openfx-arena + openfx-gmic on Linux** — the Windows package ships both, the AppImage ships only IO/Misc/CImg. Arena needs ImageMagick, poppler, librsvg, libcdr, libzip, fontconfig (not in the ASWF image: build into /usr/local like FFmpeg, cached under DEPS_KEY); G'MIC is self-contained but a very long compile (cache it the same way). Add both to `tools/linux/build-plugins.sh` + the packaging, mirroring the Windows patches/refs. Until then the README states the gap.
