...
#ifndef NATRON_ENGINE_DEEPSLICE_H
#define NATRON_ENGINE_DEEPSLICE_H
...
/**
 * @brief Extract deep samples within a Z depth range.
 *
 * Keeps only samples whose Z falls within [nearClip, farClip].
 * Samples outside this range are discarded.
 *
 * The output is a new deep image that can be passed to DeepFlatten, DeepMerge, etc.
 */
class DeepSlice
...
public:
    ...
    {
        return new DeepSlice(n);
    }

    DeepSlice(NodePtr node);
    ...
    {
        return PLUGINID_NATRON_DEEPSLICE;
    }
...
private:
    ...
    std::unique_ptr<DeepSlicePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPSLICE_H
