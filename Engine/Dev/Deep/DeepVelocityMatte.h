...
#ifndef NATRON_ENGINE_DEEPVELOCITYMATTE_H
#define NATRON_ENGINE_DEEPVELOCITYMATTE_H
...
/**
 * @brief Generate a matte based on depth velocity.
 *
 * Computes how much a sample's Z position varies relative to its
 * neighbors, indicating motion or dynamic elements.
 *
 * Output is a deep image with alpha/color modulated by the matte.
 */
class DeepVelocityMatte
...
public:
    ...
    {
        return new DeepVelocityMatte(n);
    }

    DeepVelocityMatte(NodePtr node);

    virtual ~DeepVelocityMatte();
    ...
    {
        return PLUGINID_NATRON_DEEPVELOCITYMATTE;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepVelocityMatte";
    }
...
private:
    ...
    std::unique_ptr<DeepVelocityMattePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPVELOCITYMATTE_H
