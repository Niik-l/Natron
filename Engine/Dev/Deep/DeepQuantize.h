...
#ifndef NATRON_ENGINE_DEEPQUANTIZE_H
#define NATRON_ENGINE_DEEPQUANTIZE_H
...
/**
 * @brief Snap deep sample Z values to a regular depth grid.
 *
 * Useful for regularizing irregular deep data or reducing depth
 * precision for optimization.
 *
 * The output is a deep image with quantized Z values.
 */
class DeepQuantize
...
public:
    ...
    {
        return new DeepQuantize(n);
    }

    DeepQuantize(NodePtr node);

    virtual ~DeepQuantize();
    ...
    {
        return PLUGINID_NATRON_DEEPQUANTIZE;
    }
...
private:
    ...
    std::unique_ptr<DeepQuantizePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPQUANTIZE_H
