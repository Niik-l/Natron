...
#ifndef NATRON_ENGINE_DEEPCONTACTSHADOW_H
#define NATRON_ENGINE_DEEPCONTACTSHADOW_H
...
/**
 * @brief Generate contact shadows by darkening areas where deep samples
 * from different objects are close together in Z depth.
 *
 * This creates automatic darkening at contact points between overlapping
 * geometry without requiring a separate render pass.
 */
class DeepContactShadow
...
public:
    ...
    {
        return new DeepContactShadow(n);
    }

    DeepContactShadow(NodePtr node);

    virtual ~DeepContactShadow();
    ...
    {
        return PLUGINID_NATRON_DEEPCONTACTSHADOW;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepContactShadow";
    }
...
private:
    ...
    std::unique_ptr<DeepContactShadowPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPCONTACTSHADOW_H
