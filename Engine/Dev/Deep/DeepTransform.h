...
#ifndef NATRON_ENGINE_DEEPTRANSFORM_H
#define NATRON_ENGINE_DEEPTRANSFORM_H
...
/**
 * @brief Transform deep image data in 3D space.
 *
 * Translates (X, Y, Z offset) and scales (Z scale) deep samples.
 * X/Y translation shifts the image in pixel space.
 * Z translation shifts samples in depth.
 * Z scale multiplies all depth values.
 */
class DeepTransform
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepTransform(n); }

    DeepTransform(NodePtr node);
    ...
    { return PLUGINID_NATRON_DEEPTRANSFORM; }
...
private:
    ...
    std::unique_ptr<DeepTransformPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPTRANSFORM_H
