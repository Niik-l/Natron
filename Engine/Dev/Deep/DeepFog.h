...
#ifndef NATRON_ENGINE_DEEPFOG_H
#define NATRON_ENGINE_DEEPFOG_H
...
/**
 * @brief Insert volumetric fog between deep samples at a specified depth range.
 *
 * Uses Beer-Lambert attenuation for physically-based fog density.
 * The fog correctly wraps around semi-transparent elements like hair, smoke, and glass.
 *
 * The output is a new deep image with fog samples inserted between existing samples.
 */
class DeepFog
...
public:
    ...
    {
        return new DeepFog(n);
    }

    DeepFog(NodePtr node);
    ...
    {
        return PLUGINID_NATRON_DEEPFOG;
    }
...
private:
    ...
    std::unique_ptr<DeepFogPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPFOG_H
