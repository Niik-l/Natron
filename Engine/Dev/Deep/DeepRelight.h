...
#ifndef NATRON_ENGINE_DEEPRELIGHT_H
#define NATRON_ENGINE_DEEPRELIGHT_H
...
/**
 * @brief Add point or directional lighting to a deep image using per-sample depth information.
 *
 * Normals are estimated from depth gradients in neighboring pixels.
 * This enables relighting a rendered scene in compositing without re-rendering.
 */
class DeepRelight
...
public:
    ...
    {
        return new DeepRelight(n);
    }

    DeepRelight(NodePtr node);
    ...
    {
        return PLUGINID_NATRON_DEEPRELIGHT;
    }
...
private:
    ...
    std::unique_ptr<DeepRelightPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPRELIGHT_H
