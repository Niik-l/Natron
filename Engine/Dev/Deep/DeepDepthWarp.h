...
#ifndef NATRON_ENGINE_DEEPDEPTHWARP_H
#define NATRON_ENGINE_DEEPDEPTHWARP_H
...
/**
 * @brief Non-linearly remap Z depth values in a deep image.
 *
 * Compress, expand, or shift depth ranges with artistic control.
 * Like a grade node but for the Z channel.
 */
class DeepDepthWarp
...
public:
    ...
    {
        return new DeepDepthWarp(n);
    }

    DeepDepthWarp(NodePtr node);

    virtual ~DeepDepthWarp();
    ...
    {
        return PLUGINID_NATRON_DEEPDEPTHWARP;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepDepthWarp";
    }
...
private:
    ...
    std::unique_ptr<DeepDepthWarpPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPDEPTHWARP_H
