...
#ifndef NATRON_ENGINE_DEEPSAMPLEFILTER_H
#define NATRON_ENGINE_DEEPSAMPLEFILTER_H
...
/**
 * @brief Filter deep samples by various criteria.
 *
 * Remove samples by alpha threshold, limit sample count per pixel,
 * or remove by depth range.
 *
 * The output is a new deep image with filtered samples.
 */
class DeepSampleFilter
...
public:
    ...
    {
        return new DeepSampleFilter(n);
    }

    DeepSampleFilter(NodePtr node);

    virtual ~DeepSampleFilter();
    ...
    {
        return PLUGINID_NATRON_DEEPSAMPLEFILTER;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepSampleFilter";
    }
...
private:
    ...
    std::unique_ptr<DeepSampleFilterPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPSAMPLEFILTER_H
