...
#ifndef NATRON_ENGINE_DEEPNORMALMATTE_H
#define NATRON_ENGINE_DEEPNORMALMATTE_H
...
/**
 * @brief Generate a matte based on estimated surface normal direction.
 *
 * Estimates normals from depth gradients and computes a dot product
 * against a target direction. Useful for isolating upward-facing
 * surfaces (snow, rain, dust) or side-facing surfaces.
 *
 * Output is a deep image with alpha/color modulated by the matte.
 */
class DeepNormalMatte
...
public:
    ...
    {
        return new DeepNormalMatte(n);
    }

    DeepNormalMatte(NodePtr node);

    virtual ~DeepNormalMatte();
    ...
    {
        return PLUGINID_NATRON_DEEPNORMALMATTE;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepNormalMatte";
    }
...
private:
    ...
    std::unique_ptr<DeepNormalMattePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPNORMALMATTE_H
