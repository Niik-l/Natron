...
#ifndef NATRON_ENGINE_DEEPBLEND_H
#define NATRON_ENGINE_DEEPBLEND_H
...
/**
 * @brief Blend between two deep images.
 *
 * For each sample, interpolates color values between the two inputs
 * based on the mix factor. Samples are matched by depth proximity.
 *
 * Output is a deep image containing samples from both inputs, scaled by mix.
 */
class DeepBlend
...
public:
    ...
    {
        return new DeepBlend(n);
    }

    DeepBlend(NodePtr node);
    ...
    {
        return PLUGINID_NATRON_DEEPBLEND;
    }
...
private:
    ...
    std::unique_ptr<DeepBlendPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPBLEND_H
