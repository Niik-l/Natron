...
#ifndef NATRON_ENGINE_DEEPFROMFRAMES_H
#define NATRON_ENGINE_DEEPFROMFRAMES_H
...
/**
 * @brief Convert a flat RGBA image into deep samples at a specified depth.
 *
 * Connect multiple DeepFromFrames to DeepMerge to build a deep image
 * from separate 2D renders at different depths.
 *
 * Output is a deep image with one sample per non-transparent pixel.
 */
class DeepFromFrames
...
public:
    ...
    {
        return new DeepFromFrames(n);
    }

    DeepFromFrames(NodePtr node);

    virtual ~DeepFromFrames();
    ...
    {
        return PLUGINID_NATRON_DEEPFROMFRAMES;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepFromFrames";
    }
...
private:
    ...
    std::unique_ptr<DeepFromFramesPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPFROMFRAMES_H
