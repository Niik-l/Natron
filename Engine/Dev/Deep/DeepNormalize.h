...
#ifndef NATRON_ENGINE_DEEPNORMALIZE_H
#define NATRON_ENGINE_DEEPNORMALIZE_H
...
/**
 * @brief Fix broken deep data by normalizing alpha values.
 *
 * Ensures per-sample alpha values are valid, accumulated alpha never
 * exceeds 1.0, clamps negative values, and fixes NaN/Inf.
 *
 * The output is a corrected deep image.
 */
class DeepNormalize
...
public:
    ...
    {
        return new DeepNormalize(n);
    }

    DeepNormalize(NodePtr node);

    virtual ~DeepNormalize();
    ...
    {
        return PLUGINID_NATRON_DEEPNORMALIZE;
    }
...
private:
    ...
    std::unique_ptr<DeepNormalizePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPNORMALIZE_H
