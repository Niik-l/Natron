...
#ifndef NATRON_ENGINE_DEEPLAYERBREAK_H
#define NATRON_ENGINE_DEEPLAYERBREAK_H
...
/**
 * @brief Flatten specific depth bands from a deep image.
 *
 * Set near/far depth to extract and flatten only samples within that range.
 * Like DeepSlice + DeepFlatten in one node.
 *
 * Output is a flat RGBA image.
 */
class DeepLayerBreak
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepLayerBreak(n); }

    DeepLayerBreak(NodePtr node);
    virtual ~DeepLayerBreak();
    ...
    { return PLUGINID_NATRON_DEEPLAYERBREAK; }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    { return "DeepLayerBreak"; }
...
private:
    ...
    std::unique_ptr<DeepLayerBreakPrivate> _imp;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPLAYERBREAK_H
