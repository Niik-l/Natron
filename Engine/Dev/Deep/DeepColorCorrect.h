...
#ifndef NATRON_ENGINE_DEEPCOLORCORRECT_H
#define NATRON_ENGINE_DEEPCOLORCORRECT_H
...
/**
 * @brief Color correct deep image samples with depth-range masking.
 *
 * Like DeepGrade but with a Masking tab that limits corrections to
 * samples within a specified depth range, with falloff.
 * Applies saturation, contrast, gamma, gain, and offset.
 */
class DeepColorCorrect
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepColorCorrect(n); }

    DeepColorCorrect(NodePtr node);
    virtual ~DeepColorCorrect();
    ...
    { return PLUGINID_NATRON_DEEPCOLORCORRECT; }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    { return "DeepColorCorrect"; }
...
private:
    ...
    std::unique_ptr<DeepColorCorrectPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPCOLORCORRECT_H
