...
#ifndef NATRON_ENGINE_DEEPHOLDOUT_H
#define NATRON_ENGINE_DEEPHOLDOUT_H
...
struct DeepHoldoutPrivate;

/**
 * @brief Remove deep samples that are behind (occluded by) a holdout matte.
 *
 * Input 0 ("Deep"): The deep image to apply the holdout to.
 * Input 1 ("Holdout"): A deep image used as the holdout matte. Samples in the
 *   main deep image that are behind the holdout's depth are removed or attenuated.
 */
class DeepHoldout
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepHoldout(n); }

    DeepHoldout(NodePtr node);
    virtual ~DeepHoldout();
    ...
    { return PLUGINID_NATRON_DEEPHOLDOUT; }
...
#endif // NATRON_ENGINE_DEEPHOLDOUT_H
