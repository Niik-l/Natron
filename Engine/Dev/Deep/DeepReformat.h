...
#ifndef NATRON_ENGINE_DEEPREFORMAT_H
#define NATRON_ENGINE_DEEPREFORMAT_H
...
/**
 * @brief Resize/reformat a deep image to a new resolution.
 *
 * Nearest-neighbor sampling: each output pixel maps to the nearest input pixel.
 * Deep samples are copied without modification (no interpolation of sample data).
 */
class DeepReformat
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepReformat(n); }

    DeepReformat(NodePtr node);
    virtual ~DeepReformat();
    ...
    { return PLUGINID_NATRON_DEEPREFORMAT; }
...
#endif // NATRON_ENGINE_DEEPREFORMAT_H
