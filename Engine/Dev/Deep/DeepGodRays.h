...
#ifndef NATRON_ENGINE_DEEPGODRAYS_H
#define NATRON_ENGINE_DEEPGODRAYS_H
...
/**
 * @brief Compute volumetric light shafts from deep image transmittance.
 *
 * Ray-marches from each pixel toward a light source, accumulating light
 * contribution where the deep data is transparent.
 *
 * Output is a flat RGBA image to be added over the flattened result.
 */
class DeepGodRays
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepGodRays(n); }

    DeepGodRays(NodePtr node);
    virtual ~DeepGodRays();
    ...
    { return PLUGINID_NATRON_DEEPGODRAYS; }
...
private:
    ...
    std::unique_ptr<DeepGodRaysPrivate> _imp;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPGODRAYS_H
