...
#ifndef NATRON_ENGINE_DEEPPOSITIONMATTE_H
#define NATRON_ENGINE_DEEPPOSITIONMATTE_H
...
/**
 * @brief Generate a matte from deep sample 3D positions using geometric shapes.
 *
 * Each sample's position is derived from its pixel XY and depth Z.
 * Supports sphere, cube, and cylinder shapes with optional falloff.
 *
 * Useful for isolating objects by spatial location in a deep image.
 */
class DeepPositionMatte
...
public:
    ...
    {
        return new DeepPositionMatte(n);
    }

    DeepPositionMatte(NodePtr node);

    virtual ~DeepPositionMatte();
    ...
    {
        return PLUGINID_NATRON_DEEPPOSITIONMATTE;
    }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    {
        return "DeepPositionMatte";
    }
...
private:
    ...
    std::unique_ptr<DeepPositionMattePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPPOSITIONMATTE_H
