...
#ifndef NATRON_ENGINE_DEEPGRADE_H
#define NATRON_ENGINE_DEEPGRADE_H
...
/**
 * @brief Color correct deep image samples.
 *
 * Applies gain, gamma, and offset to the RGB channels of each deep sample
 * while preserving depth and alpha structure.
 *
 * Formula: out = (in * gain + offset) ^ (1/gamma)
 */
class DeepGrade
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepGrade(n); }

    DeepGrade(NodePtr node);
    virtual ~DeepGrade();
    ...
    { return PLUGINID_NATRON_DEEPGRADE; }
...
private:
    ...
    std::unique_ptr<DeepGradePrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPGRADE_H
