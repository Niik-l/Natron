...
#ifndef NATRON_ENGINE_DEEPEXPRESSION_H
#define NATRON_ENGINE_DEEPEXPRESSION_H
...
/**
 * @brief Apply per-sample modifications to deep data using simple expressions.
 *
 * Provides multiplier and offset knobs for each channel (R, G, B, A, Z, ZBack)
 * plus a depth range filter. This is a simplified alternative to Nuke's
 * Tcl-based DeepExpression — we use knobs instead of text expressions for
 * safety and performance.
 *
 * For each sample: out_channel = in_channel * multiply + add
 * Depth range: only modify samples within [zMin, zMax]
 */
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepExpression(n); }
    ...
    { return PLUGINID_NATRON_DEEPEXPRESSION; }
...
private:
    ...
    std::unique_ptr<DeepExpressionPrivate> _imp;
    mutable DeepImagePtr _lastDeepImage;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPEXPRESSION_H
