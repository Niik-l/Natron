...
#ifndef NATRON_ENGINE_DEEPDIFFERENCE_H
#define NATRON_ENGINE_DEEPDIFFERENCE_H
...
/**
 * @brief Compute the difference between two deep images for quality control.
 *
 * Output shows where samples differ in color, alpha, or depth.
 *
 * Output is a flat RGBA image (no deep output).
 */
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new DeepDifference(n); }

    DeepDifference(NodePtr node);
    virtual ~DeepDifference();
    ...
    { return PLUGINID_NATRON_DEEPDIFFERENCE; }
    ...
    { return (inputNb == 0) ? "Deep A" : "Deep B"; }
...
private:
    ...
    std::unique_ptr<DeepDifferencePrivate> _imp;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_DEEPDIFFERENCE_H
