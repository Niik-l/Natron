...
#ifndef NATRON_ENGINE_CUBE3D_H
#define NATRON_ENGINE_CUBE3D_H
...
/**
 * @brief Cube geometry with per-face UV mapping.
 *
 * Input 0 (img): 2D image to texture onto each face.
 *
 * Generates a cube mesh with 24 vertices (4 per face) for proper
 * per-face UV mapping. Each face gets the full 0-1 UV range.
 * Connect to ScanlineRender's obj/scn input.
 *
 * Equivalent to Nuke's Cube node.
 */
class Cube3D
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new Cube3D(n); }
    ...
    // Cube mesh data for viewport and ScanlineRender
    struct CubeVertex {
        float x, y, z;
        float u, v;
        float nx, ny, nz;
