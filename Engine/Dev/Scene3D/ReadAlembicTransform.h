...
private:
    ...
    // Auto-load the transform when a saved project is restored: the file-path knob
    // is restored but knobChanged does not fire, so the transform would otherwise
    // not be read until the user clicks Reload.
