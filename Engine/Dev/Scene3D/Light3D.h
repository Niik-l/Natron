...
#ifndef NATRON_ENGINE_LIGHT3D_H
#define NATRON_ENGINE_LIGHT3D_H
...
/**
 * @brief 3D point light for illuminating volumes and geometry.
 *
 * No inputs. Provides light position, color, and intensity.
 * Connect to ScanlineRender or Scene to light the 3D scene.
 *
 * Shows as a light icon in the 3D viewport with transform gizmo.
 */
class Light3D
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new Light3D(n); }

    Light3D(NodePtr node);
    virtual ~Light3D();
    ...
    { return PLUGINID_NATRON_LIGHT3D; }
