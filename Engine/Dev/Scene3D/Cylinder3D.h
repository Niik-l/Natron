...
#ifndef NATRON_ENGINE_CYLINDER3D_H
#define NATRON_ENGINE_CYLINDER3D_H
...
/**
 * @brief Cylinder geometry with configurable rows, columns, radius, height,
 ...
 *
 * Equivalent to Nuke's Cylinder node.
 */
class Cylinder3D
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new Cylinder3D(n); }

    Cylinder3D(NodePtr node);
    virtual ~Cylinder3D();
    ...
    { return PLUGINID_NATRON_CYLINDER3D; }
    ...
    // Cylinder mesh data for viewport and ScanlineRender
    ...
    void getCylinderTransform(double time,
                              double& tx, double& ty, double& tz,
                              double& rx, double& ry, double& rz,
                              double& sx, double& sy, double& sz) const;

    void generateCylinderMesh(double time,
...
private:
    ...
    std::unique_ptr<Cylinder3DPrivate> _imp;
...
#endif // NATRON_ENGINE_CYLINDER3D_H
