...
#ifndef NATRON_ENGINE_SPHERE3D_H
#define NATRON_ENGINE_SPHERE3D_H
...
/**
 * @brief Sphere geometry with an img input for texturing.
 *
 * Input 0 (img): 2D image to texture onto the sphere (equirectangular UV mapping).
 *
 * Generates sphere mesh data with configurable rows, columns, radius.
 ...
 *
 * Equivalent to Nuke's Sphere node.
 */
class Sphere3D
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new Sphere3D(n); }

    Sphere3D(NodePtr node);
    virtual ~Sphere3D();
    ...
    { return PLUGINID_NATRON_SPHERE3D; }
    ...
    // Sphere mesh data for viewport and ScanlineRender
    struct SphereVertex {
        float x, y, z;
        float u, v;
        float nx, ny, nz;
    ...
    void getSphereTransform(double time,
                            double& tx, double& ty, double& tz,
                            double& rx, double& ry, double& rz,
                            double& sx, double& sy, double& sz) const;

    void generateSphereMesh(double time,
                            std::vector<SphereVertex>& outVertices,
                            std::vector<int>& outTriIndices) const;

    int getRows(double time) const;
    int getColumns(double time) const;
    double getRadius(double time) const;
...
private:
    ...
    std::unique_ptr<Sphere3DPrivate> _imp;
    mutable CachedTexture _cachedTexture;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_SPHERE3D_H
