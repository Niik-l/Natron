...
// ---------------------------------------------------------------------------
// ParticleTurbulence implementation
// ---------------------------------------------------------------------------
...
ParticleTurbulence::ParticleTurbulence(NodePtr node)
    : ParticleModifier(node)
    , _imp(new ParticleTurbulencePrivate())
{
}
...
std::string
ParticleTurbulence::getPluginDescription() const
{
    return tr("Applies 3D curl noise turbulence for volumetric swirling motion.\n\n"
              "Connect to a ParticleEmitter or another particle modifier.\n"
              "Uses three independent noise fields to produce true 3D divergence-free curl noise.").toStdString();
}

void
ParticleTurbulence::initializeKnobs()
{
...
void
ParticleTurbulence::applyForce(ParticleDataPtr data, double time)
{
    if (!data) return;
    ...
    for (size_t i = 0; i < data->particles.size(); ++i) {
        ...
        double z = (double)p.pz * invScale + timeOffset;

        // 3 independent noise fields using large coordinate offsets
        double n1_x = x;           double n1_y = y;           double n1_z = z;
        double n2_x = x + 31.416;  double n2_y = y + 47.853;  double n2_z = z + 12.345;
        double n3_x = x + 113.209; double n3_y = y + 97.531;  double n3_z = z + 67.890;

        // Curl = cross product of gradients of the 3 scalar fields
        // curlX = dn3/dy - dn2/dz
        float curlX = (float)((fbm3(n3_x, n3_y + eps, n3_z, octaves, lacunarity, gain) -
                               fbm3(n3_x, n3_y - eps, n3_z, octaves, lacunarity, gain)) / (2.0 * eps)
                             - (fbm3(n2_x, n2_y, n2_z + eps, octaves, lacunarity, gain) -
                               fbm3(n2_x, n2_y, n2_z - eps, octaves, lacunarity, gain)) / (2.0 * eps));

        // curlY = dn1/dz - dn3/dx
        float curlY = (float)((fbm3(n1_x, n1_y, n1_z + eps, octaves, lacunarity, gain) -
                               fbm3(n1_x, n1_y, n1_z - eps, octaves, lacunarity, gain)) / (2.0 * eps)
                             - (fbm3(n3_x + eps, n3_y, n3_z, octaves, lacunarity, gain) -
                               fbm3(n3_x - eps, n3_y, n3_z, octaves, lacunarity, gain)) / (2.0 * eps));

        // curlZ = dn2/dx - dn1/dy
        float curlZ = (float)((fbm3(n2_x + eps, n2_y, n2_z, octaves, lacunarity, gain) -
                               fbm3(n2_x - eps, n2_y, n2_z, octaves, lacunarity, gain)) / (2.0 * eps)
                             - (fbm3(n1_x, n1_y + eps, n1_z, octaves, lacunarity, gain) -
                               fbm3(n1_x, n1_y - eps, n1_z, octaves, lacunarity, gain)) / (2.0 * eps));
...
#include "moc_ParticleTurbulence.cpp"
