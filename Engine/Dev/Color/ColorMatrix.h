...
#ifndef NATRON_ENGINE_COLORMATRIX_H
#define NATRON_ENGINE_COLORMATRIX_H
...
/**
 * @brief Apply a 4x4 color matrix to RGBA channels with optional inversion.
 *
 * Similar to Nuke's ColorMatrix node. Each output channel is a linear
 * combination of input RGBA. The Invert checkbox computes the matrix
 * inverse so you can reverse a color transform.
 *
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new ColorMatrix(n); }

    ColorMatrix(NodePtr node);
    virtual ~ColorMatrix();
    ...
    { return PLUGINID_NATRON_COLORMATRIX; }
...
private:
    ...
    std::unique_ptr<ColorMatrixPrivate> _imp;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_COLORMATRIX_H
