...
#ifndef NATRON_ENGINE_EXPOSURE_H
#define NATRON_ENGINE_EXPOSURE_H
...
/**
 * @brief Adjust image exposure in stops (f-stops).
 *
 * Similar to Nuke's EXPTool. Each channel is multiplied by 2^exposure.
 * Per-channel offsets allow independent R/G/B adjustment.
 *
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new Exposure(n); }

    Exposure(NodePtr node);
    virtual ~Exposure();
    ...
    { return PLUGINID_NATRON_EXPOSURE; }
...
private:
    ...
    std::unique_ptr<ExposurePrivate> _imp;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_EXPOSURE_H
