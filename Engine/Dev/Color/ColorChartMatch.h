...
#ifndef NATRON_ENGINE_COLORCHARTMATCH_H
#define NATRON_ENGINE_COLORCHARTMATCH_H
...
/**
 * @brief Match image colors using a color reference chart (Macbeth/ColorChecker).
 *
 * Samples color patches from a photographed chart, compares against known
 * reference values (or a second chart image), and computes a best-fit 3x3
 * color correction matrix via least-squares.
 *
 * Input 0: Source image (the plate to correct, with chart visible)
 * Input 1: Target image (optional — another plate with a chart, or unused if using reference values)
 *
 * Inspired by Marco Meyer's mmColorTarget Nuke gizmo.
 */
class ColorChartMatch
...
public:
    ...
    static EffectInstance* BuildEffect(NodePtr n) { return new ColorChartMatch(n); }

    ColorChartMatch(NodePtr node);
    virtual ~ColorChartMatch();
    ...
    { return PLUGINID_NATRON_COLORCHARTMATCH; }

    virtual std::string getPluginLabel() const OVERRIDE FINAL WARN_UNUSED_RETURN
    { return "ColorChartMatch"; }

    virtual std::string getPluginDescription() const OVERRIDE FINAL WARN_UNUSED_RETURN;

    virtual void getPluginGrouping(std::list<std::string>* grouping) const OVERRIDE FINAL
    {
        grouping->push_back(PLUGIN_GROUP_COLOR);
    }
...
private:
    ...
    std::unique_ptr<ColorChartMatchPrivate> _imp;
};

NATRON_NAMESPACE_EXIT

#endif // NATRON_ENGINE_COLORCHARTMATCH_H
