...
**3b. OIIO 3.x — colorspace hints use the built-in OCIO config's names** (`OIIO/ReadOIIO.cpp`, `guessColorspace`). Not a compile fix — a runtime one, but it lives in the same patch file. OIIO 3.x tags an 8-bit PNG/JPEG with `oiio:ColorSpace = "srgb_rec709_scene"` (the built-in OCIO config's canonical name; also `lin_rec709`, `g22_rec709`, …) where 2.x wrote `"sRGB"` / `"Linear"`. `guessColorspace` only understood the legacy hints, so the new name was written verbatim into the Read's input colorspace — and with Natron's classic configs (blender / nuke-default) that name does not exist, so every Read of an 8-bit file errored `Color space 'srgb_rec709_scene' could not be found` with a text field where the colorspace menu should be. The patch adds a `legacyColorSpaceHint()` table (OIIO 3 names → `sRGB` / `Linear` / `GammaCorrected`) applied when the active config doesn't know the raw name, adds the ACES texture-space names (`sRGB - Texture`, `srgb_rec709_scene`, `Utility - sRGB - Texture`) to the sRGB lookup, and — the safety net — re-guesses as an unlabelled file whenever a hint resolves to a name the config lacks, so the bit-depth default applies instead of an error. The same ACES texture names are added to `PNG/ReadPNG.cpp`'s two sRGB lookups (gamma-2.2 and sRGB-chunk branches), which otherwise fall through to an unknown name under an ACES config. And `IOSupport/GenericOCIO.cpp`'s "reset to the default colorspace" fallback (used whenever a name is not in the config: config change, user edit, unknown guess) used the `default` ROLE and, when a config lacked it, wrote the literal string `default` into the knob — every ACES 1.x/2.x cg/studio config lacks it, so Reads and Writes under those configs errored `Color space 'default' could not be found`; the output-space branch even ignored the name it had just computed. A `fallbackColorSpaceName()` helper now tries `default`, then `scene_linear`, then `reference`, then the first active colorspace, and leaves the knob alone if all fail. Finally (2026-09-09) a **synonym table** (`kColorSpaceSynonyms` / `resolveColorSpaceName()` in `GenericOCIO.cpp`): a project saved under one config carries that config's names (`sRGB` from nuke-default/blender, `sRGB - Texture` from ACES 1.x, `srgb_rec709_scene` from ACES 2 / OIIO 3, `Linear` vs `scene_linear`, `Rec709`, `Gamma2.2`, `Cineon`, …) and the plugin only validated the name on a user edit, so opening it under another config failed at render time with `Color space 'sRGB' could not be found`. Every read of the input/output name now goes through the resolver — an unknown name is replaced by the first member of its group the active config knows — and `changedParam` stores the resolved name whatever the change reason. Natron's side: `ReadNode::onKnobsLoaded()` fires the decoder's changedParam for `ocioInputSpace` once the project's values are in the knobs, so the menu shows the resolved name instead of a text field. And the root of the recurring `'default'` (2026-09-13): `canonicalizeColorSpace()` compared an UNKNOWN name's index (-1) with each role's index and, under a config lacking the `default` role (every ACES config), -1 == -1 returned the literal `default` — so the describe-time default of ReadPNG's colorspace knob (built from `"sRGB"`, not an ACES name) was `default`, and any Read whose guess did not apply rendered with it. Fixed: unknown names are never equated with a missing role; the describe-time `sRGB` lookup knows the ACES texture names; a stale `default` resolves to the config fallback; the knob defaults go through the resolver.
