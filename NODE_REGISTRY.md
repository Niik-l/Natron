...
| **CyclesRenderPassManager** | `fr.inria.built-in.CyclesRenderPassManager` | Registered | Sink node for batched Cycles render-pass submission. Owns a JSON-backed list of pass specs (id / name / type / enabled-solo-mute-output flags / camera+vis+light+material overrides / AOV subset / file path / format-bitdepth-compression / samples). "Render to Disk" button: (1) parses + filters the active set (`enabled && output && !mute && (!soloActive \|\| solo)`); (2) resolves dollar-token paths (`$PASS` / `$SHOT` / `$RENDER` from env, frame via `####` / `%04d` / digit-group); (3) iterates the frame range per the Frame Mode knob (Current / Range / Range No Re-render); (4) groups active passes into batches keyed on shared scene state (currently `samples`); (5) runs ONE Cycles session per batch over the union of all batch members' AOVs via the shared `CyclesPassRender` helper; (6) demuxes per-pass AOV subsets and writes one multi-layer EXR per pass via `CyclesRenderer::saveMultiLayerEXR` with per-pass bit-depth (`16-bit Half` / `32-bit Full`) and compression (ZIP / ZIPS / PIZ / DWAA / DWAB / RLE / PXR24 / B44 / B44A / None). Output resolution auto-detected from the project's default format. |
