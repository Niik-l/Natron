...
// Fast Volume Render: the user's fastVolume.ntp reference scene (2026-09-15)
// as a template - a dense flattened procedural cumulus lit by two coloured
// point lights through FastVolumeRender, which previews volumes in real time
// on the GPU (no Cycles). A pathless ReadVDB sits beside the cloud as the slot
// for a real fire/smoke sequence; no .vdb ships (far too large).
void
Gui::createTemplateFastVolume()
{
    runTemplatePython(this, R"PY(
app = app1
cloud = app.createNode("fr.inria.built-in.Volume3D")
vdb = app.createNode("fr.inria.built-in.ReadVDB")
key = app.createNode("fr.inria.built-in.Light3D")
fill = app.createNode("fr.inria.built-in.Light3D")
scene = app.createNode("fr.inria.built-in.Scene3D")
camera = app.createNode("fr.inria.built-in.Camera3D")
fvr = app.createNode("fr.inria.built-in.FastVolumeRender")
viewer = app.createNode("fr.inria.built-in.Viewer")
scene.connectInput(0, cloud)
scene.connectInput(1, key)
scene.connectInput(2, fill)
fvr.connectInput(0, scene)
fvr.connectInput(1, camera)
viewer.connectInput(0, fvr)
# Layout (the reference scene's positions, re-centred on the cloud)
cloud.setPosition(0, -170)
vdb.setPosition(-330, -170)
key.setPosition(265, -175)
fill.setPosition(480, -140)
scene.setPosition(165, 0)
camera.setPosition(-65, 110)
fvr.setPosition(165, 100)
viewer.setPosition(165, 240)
# Procedural cloud: a flattened cumulus with noise, fairly dense
cloud.setLabel("Cloud (Volume3D, procedural)")
cloud.getParam("scaleX").setValue(2.2)
cloud.getParam("scaleY").setValue(1.7)
cloud.getParam("scaleZ").setValue(2.2)
cloud.getParam("resolution").setValue(200)
cloud.getParam("density").setValue(35.24)
cloud.getParam("baseFlatness").setValue(0.125)
cloud.getParam("topFlatness").setValue(0.077)
cloud.getParam("heightFalloff").setValue(0.531)
cloud.getParam("enableNoise").setValue(True)
cloud.getParam("noiseScale").setValue(0.75)
cloud.getParam("noiseDetail").setValue(5.0)
cloud.getParam("warp").setValue(0.4)
cloud.getParam("coverage").setValue(0.85)
cloud.getParam("erosion").setValue(0.763)
cloud.getParam("edgeDetail").setValue(0.044)
cloud.getParam("seed").setValue(1)
cloud.getParam("stepSize").setValue(0.255)
cloud.getParam("volumeBounces").setValue(3)
# VDB slot: pathless, unconnected - swap it for the cloud when you have a sequence
vdb.setLabel("ReadVDB - optional: fire/smoke .vdb goes here (then wire to Scene input 4)")
# Two coloured point lights: cool blue key from the right/behind, warm red fill from the left
key.setLabel("Key (blue)")
key.getParam("translateX").setValue(8.09)
key.getParam("translateY").setValue(4.24)
key.getParam("translateZ").setValue(-4.32)
key.getParam("lightColor").setValue(0.0, 0)
key.getParam("lightColor").setValue(0.0294, 1)
key.getParam("lightColor").setValue(0.61, 2)
fill.setLabel("Fill (red)")
fill.getParam("translateX").setValue(-5.76)
fill.getParam("translateZ").setValue(-8.04)
fill.getParam("lightColor").setValue(1.0, 0)
fill.getParam("lightColor").setValue(0.0504, 1)
fill.getParam("lightColor").setValue(0.0273, 2)
camera.getParam("translateY").setValue(0.825)
camera.getParam("translateZ").setValue(14.36)
fvr.getParam("sigma").setValue(0.486)
fvr.getParam("lightFactor").setValue(2)
)PY");
}
